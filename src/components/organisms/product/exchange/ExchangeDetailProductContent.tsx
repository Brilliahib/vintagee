"use client";

import { Card, CardContent, CardHeader, CardTitle } from "@/components/ui/card";
import { useGetDetailProduct } from "@/http/product/get-detail-product";
import Image from "next/image";

interface ExchangeDetailProductContentProps {
  id: string;
}

export default function ExchangeDetailProductContent({
  id,
}: ExchangeDetailProductContentProps) {
  const { data } = useGetDetailProduct(id);
  return (
    <div>
      <Card>
        <CardHeader>
          <CardTitle>Ringkasan Produk Yang Ditawar</CardTitle>
        </CardHeader>
        <CardContent>
          <div className="mb-6 md:mb-8">
            <Image
              src={data?.data.image_url ?? ""}
              alt={data?.data.name ?? ""}
              width={1000}
              height={1000}
              className="h-[300px] w-full rounded-md object-cover"
            />
          </div>
          <div className="space-y-2">
            <div className="flex flex-col gap-1 md:flex-row">
              <div className="text-muted-foreground md:w-4/12">Nama</div>
              <div className="md:w-8/12">{data?.data.name}</div>
            </div>
            <div className="flex flex-col gap-1 md:flex-row">
              <div className="text-muted-foreground md:w-4/12">Ukuran</div>
              <div className="md:w-8/12">{data?.data.size}</div>
            </div>
            <div className="flex flex-col gap-1 md:flex-row">
              <div className="text-muted-foreground md:w-4/12">Deskripsi</div>
              <div className="md:w-8/12">{data?.data.description}</div>
            </div>
            <div className="flex flex-col gap-1 md:flex-row">
              <div className="text-muted-foreground md:w-4/12">Kondisi</div>
              <div className="md:w-8/12">{data?.data.condition}</div>
            </div>
            <div className="flex flex-col gap-1 md:flex-row">
              <div className="text-muted-foreground md:w-4/12">Kategori</div>
              <div className="md:w-8/12">{data?.data.category.name}</div>
            </div>
            <div className="flex flex-col gap-1 md:flex-row">
              <div className="text-muted-foreground md:w-4/12">Brand</div>
              <div className="md:w-8/12">
                {data?.data.brand ?? "Tidak memiliki brand"}
              </div>
            </div>
          </div>
        </CardContent>
      </Card>
    </div>
  );
}
